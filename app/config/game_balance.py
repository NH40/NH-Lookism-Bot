...
# ── Массовые операции со статистами (покупка/обмен) ────────────────────────────
# Крупные INSERT/UPDATE по squad_members (покупка миллионов статистов, обмен
# в клане) режутся на батчи такого размера с промежуточными commit — иначе
# одна гигантская транзакция генерирует WAL-всплеск и Postgres уходит в
# постоянные checkpoint'ы, подвешивая базу для всех.
SQUAD_BULK_CHUNK_SIZE: int = 200_000
