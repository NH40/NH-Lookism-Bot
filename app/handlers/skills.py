...
from app.data.skills import MASTERY, MASTERY_BY_ID, PATH_SKILLS, PATH_SYNERGIES
from app.constants.raid import PATH_SPIN_CRAFT_COST, PATH_LEVEL_MAX, PATH_LEVEL_COSTS, PATH_LEVEL_BONUSES
...
async def cb_path_menu(cb: CallbackQuery, session: AsyncSession, user: User):
    ...
    # Навыки основного пути (только непокупленные)
    for skill in skills:
...
async def cb_extra_skills_menu(cb: CallbackQuery, session: AsyncSession, user: User):
    ...
    extra_count = await skill_service.get_extra_path_skills_count(session, user)
    slots_full = extra_count >= slots
    can_buy_more = not slots_full and slots > 1  # покупка за 5× только при доп. слотах
    ...
    # Определяем какие синергии активны
    from app.services.skill_service import _all_path_skills_map
