...
from app.constants.auction import (
    ...
    TIER_WEIGHTS, AUCTION_TIERS,
    AUCTION_PAUSE_MIN, AUCTION_PAUSE_MAX,
)
from .rewards import _generate_reward, _deliver_reward
...
class AuctionService:
    ...
    async def start_new_auction(self, session: AsyncSession) -> Auction | None:
        ...
        reward_data = await _generate_reward(tier)
        lot = AuctionLot(
    ...
    async def _generate_reward(self, tier: int) -> str:
        return await _generate_reward(tier)
    ...
    async def tick(self, session: AsyncSession) -> dict | None:
        ...
        if auction.winner_id and lot and not lot.is_delivered:
            ...
            if winner:
                ...
                await _deliver_reward(session, winner, lot)
        ...
        if current_round >= total_rounds:
            ...
            new_reward = await _generate_reward(auction.tier)
            new_lot = AuctionLot(
                auction_id=auction.id,
                reward_type=cfg["reward_type"],
    ...
    async def _deliver_reward(
        ...
        await _deliver_reward(session, user, lot)
