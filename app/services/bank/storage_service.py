    ...
    async def fee_tick(self, session: AsyncSession) -> None:
        ...
        for cell in cells:
            ...
            if user.nh_coins >= FEE_PER_MINUTE:
                ...
                # Не хватает монет — по требованию НХ ячейки больше НЕ
                # обнуляются от нехватки денег (это раньше било по игрокам,
                # у которых после поражения в PvP/обнуления мощи проседал
                # доход): просто пропускаем списание в этот тик, содержимое
                # остаётся на месте, плата спишется как только появятся
                # монеты. Явный сброс ячеек — только при удалении банды,
                # пробуждении и патче (см. prestige_service._reset_progress).
                logger.info(
                    f"storage_fee: user {cell.user_id} slot {cell.slot} "
                    f"— не хватило монет, плата пропущена"
                )
